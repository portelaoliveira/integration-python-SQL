...
connection_data = "Driver={SQLite3 ODBC Driver};Server=localhost;Database=Estoque.db"
...
def deletar_insumo():
    if len(nome_insumo.get()) < 2:
        ...
        # escrever na caixa de texto
        caixa_texto.insert("1.0", f"{nome_insumo.get()} inválido!")
        return
...
def consumir_insumo():
    if len(nome_insumo.get()) < 2 or len(lote_insumo.get()) < 1:
        ...
        # escrever na caixa de texto
        caixa_texto.insert("1.0", f"{nome_insumo.get()} e {lote_insumo.get()} inválido!")
        return
...
def visualizar_insumo():
    ...
    text = ""
    for _, nome, quantidade, validade, lote in values:
        text = text + f"""
        |---------------------------
        |Produto: {nome}
